...
def get_returns_data(tickers: list, start=None, end=None, max_period=True, interval='1wk', dividends=True, file_directory=None,
                      replace_tickers=True, index_freq=None, names_directory=None):
    """
    ...
    Parameters:
        ...
        names_directory (str, optional): csv file containing tickers and long name.
    ...
    for ticker in tickers:
        ...
        if replace_tickers and names_directory:
            names = pd.read_csv(names_directory, index_col='Ticker')
...
def weights_change(weights, *args, **kwargs):
    ...
    if len(weights.columns) == 1:
        col = weights['Weights'].iloc[0].keys()
        df = pd.DataFrame(columns=col, index = weights.index)
        for date in df.index:
            df.loc[date] = weights['Weights'].loc[date]
        result_df = df
    else:
